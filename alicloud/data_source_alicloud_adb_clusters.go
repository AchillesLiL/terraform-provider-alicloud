...
func dataSourceAlicloudAdbClusters() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAlicloudAdbClustersRead,
...
func dataSourceAlicloudAdbClustersRead(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("tags"); ok {
		var reqTags []adb.DescribeDBClustersTag
		for key, value := range v.(map[string]interface{}) {
			reqTags = append(reqTags, adb.DescribeDBClustersTag{
				Key:   key,
				Value: value.(string),
			})
		}
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_adb_clusters", request.GetActionName(), AlibabaCloudSdkGoERROR)
		}
		...
		for _, item := range response.Items.DBCluster {
			...
			if descriptionRegex != nil && !descriptionRegex.MatchString(item.DBClusterDescription) {
				continue
			}
...
func adbClustersDescription(d *schema.ResourceData, dbi []adb.DBClusterInDescribeDBClusters) error {
	...
	for _, item := range dbi {
		mapping := map[string]interface{}{
			...
			"vpc_id":          item.VPCId,
			"db_node_count":   item.DBNodeCount,
			"db_node_class":   item.DBNodeClass,
			"db_node_storage": item.DBNodeStorage,
		}
